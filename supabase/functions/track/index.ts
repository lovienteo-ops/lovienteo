...
function parseDevice(ua: string): { device_type: string; os: string; browser: string } {
  ...
  if (/mobile|android.*mobile|iphone|ipod|blackberry|windows phone/.test(u)) device_type = "mobile";
  else if (/ipad|android(?!.*mobile)|tablet/.test(u)) device_type = "tablet";
...
Deno.serve(async (req: Request) => {
  try {
    ...
    const { type, page, sid, ts, ua: clientUa, ref: clientRef, btn_position, ...rest } = body as {
      type: string;
      page: string;
      sid: string;
      ts: number;
