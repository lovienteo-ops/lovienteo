  ...
  try {
    ...
    if (req.method === "GET" && pathname === "/stats") {
      ...
      const { data: clickRows } = await client
        .from("analytics_events")
        .select("payload")
        .eq("page", slug).eq("type", "click")
        .gte("created_at", from).lte("created_at", to)
        .limit(500);

      const heatmapPoints = (clickRows || []).map((r: { payload: { x_pct?: number; y_pct?: number; label?: string } }) => ({
        x: r.payload?.x_pct || 0,
        y: r.payload?.y_pct || 0,
      ...
      const { data: exitRows } = await client
        ...
        .eq("page", slug).eq("type", "exit")
        .gte("created_at", from).lte("created_at", to);

      const avgTimeMs = exitRows && exitRows.length > 0
        ? Math.round(exitRows.reduce((s: number, r: { payload: { time_ms?: number } }) => s + (r.payload?.time_ms || 0), 0) / exitRows.length)
        : 0;
