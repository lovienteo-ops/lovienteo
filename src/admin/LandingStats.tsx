  ...
  return (
    <div className="dg-wrap">
      <div className="dg-toolbar">
        ...
        />
        <span className="dg-count">{filtered.length} registro{filtered.length !== 1 ? 's' : ''}</span>
      </div>
    ...
    <div className="landing-stats">
      ...
      {tab === 'sections' && (
        <div className="tab-content">
          <div className="chart-card wide">
            ...
            <BarChart
              data={sectionDwell.map(s => ({
                label: s.section.replace('section-','').replace('section_','').replace('block_','B'),
          ...
          <div className="chart-card wide">
            <div className="chart-title">Detalle por sección</div>
            <div className="section-table">
              <div className="st-header">
                <span>Sección</span><span>Vistas</span><span>Tiempo prom.</span>
              </div>
              {sectionDwell.map(s => (
                <div key={s.section} className="st-row">
                  <span className="st-name">{s.section.replace('section-','').replace('section_','')}</span>
                  <span>{s.views}</span>
                  <span className="st-time">{fmt(Math.round(s.avg_ms / 1000))}</span>
                </div>
              ))}
              {!sectionDwell.length && <div className="chart-empty">Sin datos aún</div>}
            </div>
          </div>
        </div>
      )}
