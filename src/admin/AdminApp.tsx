...
export default function AdminApp() {
  ...
  const fetchLandings = useCallback(async () => {
    ...
    try {
      ...
      setLandings(data);
      if (data.length && !activeLanding) setActiveLanding(data[0]);
    } catch { /* ignore */ }
        ...
        {landingsLoading ? (
          ...
          <nav className="sidebar-nav">
            {landings.map(l => (
              <button
                key={l.id}
                className={`sidebar-item ${activeLanding?.id === l.id ? 'active' : ''}`}
                onClick={() => { setActiveLanding(l); setSidebarOpen(false); }}
              >
      ...
      <main className="admin-main">
        ...
        <div className="admin-topbar">
          ...
          <div className="admin-topbar-title">
            {activeLanding ? activeLanding.name : 'Seleccioná una landing'}
          </div>
